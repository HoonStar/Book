...
// POST {action:"create"|"join"|"cheer"|"note", ...}
import { bookById, hasQuiz } from "../lib/engine.js";
...
async function create(b, res) {
  ...
  if (!book) return bad(res, "존재하지 않는 도서입니다.");
  if (!hasQuiz(book.id)) return bad(res, "이 책은 아직 인증 퀴즈가 없어 레이스를 만들 수 없어요.");
...
async function getState(req, res) {
  ...
  return res.status(200).json({
    ...
    book: { id: book.id, title: book.title, author: book.author, pages: book.pages },
    checkpoints: CHECKPOINTS,
    members: members.map((m) => ({ nickname: m.nickname, verified_pct: m.verified_pct, attempts: m.attempts, is_me: m.device_id === String(req.query.device || "") })),
